      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative timeline-line">
          {timelineData.map((item, index) => (
            <div
              key={index}
              className={`timeline-item fade-up flex items-center mb-16 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'
                }`}
              style={{ animationDelay: `${index * 0.2}s` }}
            >
              <div className={`w-full md:w-5/12 ${index % 2 === 0 ? 'md:pr-8' : 'md:pl-8'}`}>
                <div className="bg-gray-800/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-700 hover-lift">
                  <div className={`flex items-center mb-4 ${item.color === 'cyan' ? 'text-cyan-400' :
              ...
              <div className="hidden md:flex w-2/12 justify-center">
                <div className={`w-12 h-12 rounded-full border-4 flex items-center justify-center ${item.color === 'cyan' ? 'bg-cyan-500 border-cyan-400' :
